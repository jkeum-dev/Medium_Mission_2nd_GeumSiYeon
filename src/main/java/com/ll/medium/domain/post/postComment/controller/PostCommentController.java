...
public class PostCommentController {
	...
	public String write(@PathVariable long id, @Valid WriteForm form) {
		...
		return rq.redirect(
				"/post/" + id + "#postComment-" + postComment.getId(),
				"댓글이 작성되었습니다."
		);
	}
	...
	public String showModify(@PathVariable long id, @PathVariable long commentId) {
		...
		if (!postService.canModifyComment(rq.getMember(), postComment)) throw new GlobalException("403-1", "권한이 없습니다.");
	...
	public String modify(@PathVariable long id, @PathVariable long commentId, @Valid ModifyForm form) {
		PostComment postComment = postService.findCommentById(commentId).orElseThrow(() -> new GlobalException("404-1", "해당 댓글이 존재하지 않습니다."));
	...
	public String delete(@PathVariable long id, @PathVariable long commentId) {
		...
		if (!postService.canDeleteComment(rq.getMember(), postComment)) throw new GlobalException("403-1", "해당 댓글을 수정할 권한이 없습니다.");
